...
public class TransferService : ITransferService
{
    ...
    public async Task<TransferDto> ReceiveTransferAsync(Guid transferId, ReceiveTransferDto dto, Guid receivedBy)
    {
        ...
        foreach(var transferItem in transfer.TransferItems)
        {
            ...
            // 2a. Ghi nh?n s? lu?ng th?c nh?n lên TransferItem
            transferItem.ShippedQuantity = incoming.ShippedQuantity;
            transferItem.ReceivedQuantity = receivedQuantity;  // ? T? tính
            transferItem.DamagedQuantity = incoming.DamagedQuantity;
            transferItem.Notes = incoming.Notes ?? transferItem.Notes;

            // 3. C?ng hàng t?t vào inventories kho dích (ToLocation)
            //    netReceived = received - damaged (hàng th?c s? nh?p kho)
            if (receivedQuantity > 0)
            {
                ...
                {
                    // C?ng vào quantity — AvailableQuantity t? tính (computed = quantity - reserved)
                ...
                {
                    // T?o m?i b?n ghi t?n kho n?u s?n ph?m chua có t?i kho dích
            ...
            // 4. C?p nh?t ProductBatch.WarehouseId sang kho dích n?u có BatchId
            ...
            // 5. Gom StockMovementItem — quantity = netReceived (dã tr? hàng hu)
        ...
        // 5. T?o StockMovement INBOUND ghi nh?n t?i kho dích
        ...
        // 7. C?p nh?t RestockRequest ? COMPLETED n?u có liên k?t
        if (transfer.RestockRequestId.HasValue)
        {
            var restockRequest = await _restockRequestRepository.GetByIdAsync(transfer.RestockRequestId.Value);
    ...
    public async Task<bool> CreateOutboundStockMovementAsync(Guid transferId, Guid shippedBy)
    {
        ...
        // 2. Gi?m quantity và reserved_quantity t?i kho ngu?n
        ...
        var stockMovement = new StockMovement
        {
            ...
            TransferId = transfer.Id,
            RestockRequestId = transfer.RestockRequestId,
            ReceivedBy = shippedBy, // Ngu?i giao hàng (outbound)
            Status = "COMPLETED",
            Notes = null,
            CreatedAt = DateTime.UtcNow,
            StockMovementItems = stockMovementItems,
